...
#if defined(NDEBUG)
# error "Xenblocks cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Xenblocks Signed Message:\n";
                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s XNB additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    */
    ...
    // Pre-mine for Xenblocks Foundation
    if (nHeight == 2) {
        nSubsidy = 500000000 * COIN;
    }

    // half every 4 years
    int blocksPerYear = 1051200;
    int yearsToHalfSubsidy = 4;
    nSubsidy >>= (nHeight / (blocksPerYear * yearsToHalfSubsidy));
...
void ThreadScriptCheck() {
    RenameThread("xenblocks-scriptch");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, CBlockIndex * const pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Xenblocks: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // Xenblocks: (mainnet >= 710000, regtest and testnet uses supermajority)
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    if (nCheckDepth <= 0)
        // Xenblocks: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
